...
const Navbar = () => {
  ...
  return (
    <div className="py-7 shadow">
      <div className="container flex justify-between items-center">
        {/* navegators */}
        <nav className="navlink flex gap-4 text-gray-400">
          <NavLink to={"/"}>
            <p className="nav-text px-3 py-2 rounded-xl">Home</p>
          </NavLink>
          <NavLink to={"/shop"}>
            <p className="nav-text px-3 py-2 rounded-xl">Shop</p>
          </NavLink>
          <NavLink to={"/aboutUs"}>
            <p className="nav-text px-3 py-2 rounded-xl">About Us</p>
          </NavLink>
          <NavLink to={"/contactUs"}>
            <p className="nav-text px-3 py-2 rounded-xl">Contact Us</p>
          </NavLink>
        </nav>
        ...
        <NavLink to={"/"} className="logo flex items-center gap-2">
          ...
          <p className="text-2xl font-bold text-black">Ecobazar</p>
        </NavLink>

        {/* controll center */}
        <div className="cart flex items-center gap-4">
          <IoHeartOutline className="text-4xl cursor-pointer" />
          <div className="relative pl-4 flex items-center gap-4 cursor-pointer border-l-2 border-[#ccc]">
            <p className="absolute -top-1 left-9 w-5 h-fit rounded-full text-sm text-center font-thin text-white bg-green-700">
              0
            </p>
            <PiShoppingCartSimple className="text-3xl" />
          </div>
          <div className="relative pl-4 flex gap-4 cursor-pointer border-l-2 border-[#ccc]">
            <IoPersonOutline
              onClick={() => {
                toggelHidden();
              }}
            ...
            {
              <div
                className={`absolute top-10 right-2 ${isHidden ? "w-0 h-0" : "w-30 h-fit"} p-2 flex flex-col gap-2 bg-[#eee] rounded-xl text-center overflow-hidden duration-300`}
              >
                <Link to={"/login"} className="px-3 py-2 rounded-xl bg-[#aaa]">
                  Login
                </Link>
                <Link
                  to={"/register"}
                  className="px-3 py-2 rounded-xl bg-[#aaa]"
                >
                  Register
                </Link>
              </div>
